...
import { ReturnTipTapComponents } from '@/app/api/AddTiptap';

export default function Canvas() {
  const canvasRef = useRef<HTMLDivElement>(null);
  const [transform, setTransform] = useState({ x: 0, y: 0, scale: 1 });
  const [isDragging, setIsDragging] = useState(false);
  const [components, setComponents] = useState(ReturnTipTapComponents());
  ...
  const refreshComponents = () => {
    setComponents(ReturnTipTapComponents());
  };
  ...
  return (
    <div className='w-dvw h-dvh bg-slate-200'>
      <SideBarNew onVariablesAdd={refreshComponents} />
      <div
        ref={canvasRef}
